...
\newcommand{\datarepo}{https://doi.org/10.5281/zenodo.20805195}
...
\textbf{Repositório dos dados:} \url{\datarepo}\\
...
Foram usadas três famílias de dados públicos. O SIH/SUS fornece as Autorizações de Internação Hospitalar; o CNES identifica estabelecimentos, natureza pública/SUS e região de saúde; o IBGE fornece códigos territoriais e apoio geográfico \cite{datasus_sih,cnes,ibge_localidades}. As tabelas derivadas usadas no relatório, nos algoritmos e no dashboard foram publicadas em repositório de dados com DOI \cite{sus_graph_dataset}. A coleta e a preparação foram organizadas de forma modular: os scripts aceitam combinações de ano, meses e UFs, permitindo reproduzir a análise nacional ou gerar recortes menores para validação.
...
As transferências exigiram maior cuidado porque a base não oferece, de forma limpa e universal, uma lista explícita de origem e destino, nem um identificador individual perfeito do paciente. A implementação em \ghfile{data_layer/transfer_matching.py}{transfer\_matching.py} adotou uma heurística conservadora. Primeiro, cada internação recebe uma chave de \textit{paciente provável}, construída com data de nascimento, sexo, idade e município de residência quando esses campos estão disponíveis. Essa chave não prova identidade individual; ela apenas reduz o espaço de busca para internações que provavelmente pertencem à mesma pessoa.

Depois, o processamento em \texttt{pandas} agrupa a tabela por essa chave provável. Para cada grupo, as internações são ordenadas por data de entrada. Quando uma internação possui sinal administrativo de transferência, seja por campo próprio ou por motivo de saída validado, o algoritmo procura somente dentro do mesmo grupo uma nova internação cuja entrada ocorra entre 24 e 48 horas após a saída da internação de origem. Portanto, o algoritmo não compara cada caso com todos os hospitais do Brasil; ele procura candidatos apenas entre internações da mesma chave provável e dentro da janela temporal definida.

Os candidatos restantes passam por continuidade clínica: o capítulo CID-10 da nova internação deve ser compatível com o capítulo CID-10 da internação de origem. O CID-10 é a Classificação Internacional de Doenças, usada nos registros hospitalares para codificar diagnósticos; neste trabalho, ele foi usado apenas de forma agregada para verificar se a internação seguinte parecia pertencer ao mesmo quadro clínico. Quando havia múltiplos candidatos, a escolha priorizava menor intervalo temporal, CID exato e identificador de destino estável. O hospital destino da transferência inferida é o CNES da internação candidata escolhida. Assim, a aresta hospital $\rightarrow$ hospital deve ser lida como continuidade provável agregada, útil para padrões estruturais recorrentes, e não como rastreamento individual perfeito.
